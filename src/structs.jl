...
function movimentoValido(prn, gpu)
    global listaGPU
    global contTipoGPU

    temTipo = contTipoGPU[gpu.id, prn.tipo] > 0
    temEspaco = listaGPU[gpu.id].capacidadeRestante >= prn.custo

    return temTipo && temEspaco
end

function movePRN(prn, gpuDestino)
    global listaGPU
    global contTipoGPU
...
function trocaValida(prn1, prn2)
    global listaGPU
    global contTipoGPU

    return prn1.tipo != prn2.tipo && 
           listaGPU[prn1.gpuID].capacidadeRestante + prn1.custo >= prn2.custo &&
           listaGPU[prn2.gpuID].capacidadeRestante + prn2.custo >= prn1.custo
end

function trocaPRNs(prn1, prn2)
    global listaGPU
    global contTipoGPU
